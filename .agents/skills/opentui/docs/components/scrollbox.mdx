---
title: ScrollBox
description: Scroll and cull child content in a bounded viewport
---

# ScrollBox

`ScrollBox` scrolls arbitrary child renderables and manages its own [ScrollBar](/docs/components/scrollbar) instances. Use a standalone ScrollBar only when another model owns the scroll position.
...
behavior. If the child already fits, the call is a no-op. An oversized child also stays in place when it extends beyond
both edges of an axis. Otherwise, the method scrolls to reveal the nearest edge.

```typescript
scrollbox.scrollChildIntoView("table-row-42");
```

Use it when you focus an offscreen element, such as a search result or a new form field. The method moves the scroll
position only as much as necessary.
...
Read [Layout](/docs/core-concepts/layout) for viewport sizing. Read [Interaction, focus, and selection](/docs/core-concepts/interaction) for focus, wheel input, drag selection, and event propagation.
